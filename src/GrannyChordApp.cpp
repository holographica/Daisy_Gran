...
DTCMRAM_BSS WavWriter<16384> sd_out_writer_;

DSY_SDRAM_BSS Reverb GrannyChordApp::reverb_;
...
void GrannyChordApp::Init(DaisyPod& pod, GranularSynth& synth, AudioFileManager& filemgr, int16_t *left, int16_t *right){
  pod_ = &pod;
  synth_ = &synth;
  filemgr_ = &filemgr;
  left_buf_ = left;
...
void GrannyChordApp::PassCPUMeter(CpuLoadMeter& meter){
  cpumeter_ = &meter;
}

void GrannyChordApp::PrintCPULoad(){

        /* get current load (smoothed value and peak values) */
        const float avgLoad = cpumeter_->GetAvgCpuLoad();
        const float maxLoad = cpumeter_->GetMaxCpuLoad();
        const float minLoad = cpumeter_->GetMinCpuLoad();
        DebugPrint(*pod_,"Processing Load %:");
        DebugPrint(*pod_,"Max: " FLT_FMT3, FLT_VAR3(maxLoad * 100.0f));
        DebugPrint(*pod_,"Avg: " FLT_FMT3, FLT_VAR3(avgLoad * 100.0f));
        DebugPrint(*pod_,"Min: " FLT_FMT3, FLT_VAR3(minLoad * 100.0f));
}
...
void GrannyChordApp::UpdateUI(){
  ...
  if (encoder_inc!=0 && curr_state_== AppState::SelectFile){
...
void GrannyChordApp::HandleStateChange(){
  switch(curr_state_){
    // case AppState::SelectFile:  HandleFileSelection(); break; 
    case AppState::RecordIn:    ClearAudioBuffers();    break;
    case AppState::PlayWAV:     InitPlayback();        break;
    case AppState::Synthesis:   InitSynth();           break;
    case AppState::Error: 
      pod_->led1.SetRed(1); 
      pod_->led2.SetRed(1); 
      break;
...
void GrannyChordApp::HandleEncoderPressed(){
  switch(curr_state_){
    ...
    case AppState::RecordIn: 
      ...
      break;
    case AppState::PlayWAV:   curr_state_ = AppState::Synthesis; break;
    default:
      break;
  }
}
...
bool GrannyChordApp::InitFileMgr(){
  ...
  DebugPrint(*pod_,"scanning wavs now");
...
void GrannyChordApp::InitSynth(){
  ...
  synth_->Init(*pod_, left_buf_, right_buf_, filemgr_->GetSamplesPerChannel());
}
...
void GrannyChordApp::InitFX(){
  ...
  reverb_.init(SAMPLE_RATE_FLOAT, false);
...
ITCMRAM_TEXT void GrannyChordApp::AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size){
  instance_->ProcessAudio(in,out,size);
}

ITCMRAM_TEXT void GrannyChordApp::ProcessAudio(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size){
...
/// @param out Output audio buffer
/// @param size Number of samples to process in this call
ITCMRAM_TEXT void GrannyChordApp::ProcessWAVPlayback(AudioHandle::OutputBuffer out, size_t size){
  for (size_t i=0; i<size; i++){
...
/// @param out Output audio buffer
/// @param size Number of samples to process in this call
ITCMRAM_TEXT void GrannyChordApp::ProcessRecordIn(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size){
...
/// @param out Output audio buffer
/// @param size Number of samples to process in this call
ITCMRAM_TEXT void GrannyChordApp::ProcessSynthesis(AudioHandle::OutputBuffer out, size_t size){
    synth_->ProcessGrains(out[0],out[1],size); 
    ProcessFX(out, size);
}

/// @brief Process audio through onboard effects - compression, reverb, filters
/// @param out output audio buffer
/// @param size Number of samples to process in this call
ITCMRAM_TEXT void GrannyChordApp::ProcessFX(AudioHandle::OutputBuffer out, size_t size){
  comp_.ProcessBlock(out[0],out[0],size);
  comp_.ProcessBlock(out[1],out[1],size);
  float* input[2] = {out[0],out[1]};
  float* rev_out[2] = {out[0],out[1]};
  // reverb_.prepare(size);
  reverb_.process(size, input, rev_out);
  for (size_t i=0; i<size; i++){
...
void GrannyChordApp::UpdateKnob1Params(float knob1_val, SynthMode mode){
  switch (mode){
    ...
    case SynthMode::Reverb:
      /* set reverb length */
      reverb_.set_rtmid(fmap(knob1_val, 1.0f, 6.0f));
      reverb_.set_rtlow(1.2f * fmap(knob1_val, 1.0f, 6.0f));
      /* set predelay */
      reverb_.set_delay(fmap(knob1_val,0.02f, 0.10f));
      /* set high freq damping - */
      reverb_.set_fdamp((8000.0f-(knob1_val*4000.0f)));
      reverb_.prepare(pod_->AudioBlockSize());
      break;

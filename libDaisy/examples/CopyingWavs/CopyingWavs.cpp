...
void copy_wav(const char* filename)
{
    ...
    do
    {
        ...
        {
            hw.seed.qspi.Erase(current_qspi_offset, current_qspi_offset + SECTOR_SIZE);
        }
        f_read(&file, data_buffer, DATA_BUFFER_LEN, &data_read);

        hw.seed.qspi.Write(current_qspi_offset, data_read, data_buffer);
